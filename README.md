...
- `Utils/StructurePreviewer/stronghold_pieces`、`mansion_pieces`、`end_city_pieces`、`fortress_pieces`、`pyramid_pieces`、`ruined_portal_pieces` —— 要塞/林地府邸/末地城/下界堡垒/沙漠神殿+丛林神庙/废弃传送门+埋藏的宝藏的结构件生成（递归/队列组装或 postProcess 逐行转写，复刻游戏逐件 RNG 消耗）
...
- `assets/StructurePreviewer/gui`、`items` —— 容器槽位/面板纹理与 346 张物品图标
