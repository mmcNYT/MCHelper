# -*- coding: utf-8 -*-
"""构建 MapPreviewer 地图采样扩展（_map_sampler.pyd）。
...
产物：Utils/MapPreviewer/_native/_map_sampler.pyd
    缺失或加载失败时 map_sampler.py 自动降级纯 Python 路径。
"""
...
CPP = os.path.join(NATIVE_DIR, "_map_sampler.cpp")
PYD = os.path.join(NATIVE_DIR, "_map_sampler.pyd")
LOG = os.path.join(NATIVE_DIR, "_build_map_log.txt")
...
def main():
    ...
    cmd = ["cl", *FLAGS, CPP, f"/Fe:{PYD}",
           f"/Fo:{os.path.join(NATIVE_DIR, '_map_sampler.obj')}", *LINK]
